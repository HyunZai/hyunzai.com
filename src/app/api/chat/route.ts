...
export async function POST(req: NextRequest) {
    try {
        ...
        // 김현재라는 사용자의 정보만 가져온다고 가정 (ID: 1 or name based)
        ...
        if (!user) {
            return NextResponse.json({ error: "Portfolio owner data not found" }, { status: 404 });
        }
        ...
        const context = `
            ...
            Careers:
            ${user.careers.map(c => `- ${c.company} (${c.position}): ${c.description} (${c.startDate} ~ ${c.endDate || 'Present'}) [Stack: ${c.techStack}]`).join('\n')}

            Projects:
            ${user.projects.map(p => `- ${p.title}: ${p.description} (Stack: ${p.techStack}) [Demo: ${p.demoLink}]`).join('\n')}

            Personal Info / TMI:
            ${user.personalInfos.map(i => `- [${i.category}] ${i.keyName}: ${i.content}`).join('\n')}

            Milestones:
            ${user.milestones.map(m => `- ${m.startDate} ~ ${m.endDate || 'Present'}: ${m.title} (${m.organization})`).join('\n')}
        `;
